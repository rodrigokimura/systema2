...
class WhiteboardNotFoundError(LookupError):
    def __init__(self, whiteboard_id: str) -> None:
...
def get_whiteboard(session: Session, whiteboard_id: str) -> Whiteboard | None:
...
def update_whiteboard(
    session: Session, whiteboard_id: str, payload: WhiteboardUpdate
) -> Whiteboard | None:
...
def delete_whiteboard(session: Session, whiteboard_id: str) -> bool:
    wb = session.get(Whiteboard, whiteboard_id)
    if wb is None:
        return False
...
def list_boxes(session: Session, whiteboard_id: str) -> list[Box]:
    return list(
        session.exec(
            select(Box)
...
def get_box(session: Session, box_id: str) -> Box | None:
...
def update_box(
    session: Session, box_id: str, payload: BoxUpdate
) -> Box | None:
...
def delete_box(session: Session, box_id: str) -> bool:
    box = session.get(Box, box_id)
    if box is None:
        return False
...
def list_connectors(session: Session, whiteboard_id: str) -> list[Connector]:
    return list(
        session.exec(
            select(Connector)
...
def update_connector(
    session: Session, connector_id: str, payload: ConnectorUpdate
) -> Connector | None:
...
def get_whiteboard_std(whiteboard_id: str) -> Whiteboard | None:
    with _session() as s:
...
def update_whiteboard_std(
    whiteboard_id: str, payload: WhiteboardUpdate
) -> Whiteboard | None:
    with _session() as s:
        return update_whiteboard(s, whiteboard_id, payload)


def delete_whiteboard_std(whiteboard_id: str) -> bool:
    with _session() as s:
        return delete_whiteboard(s, whiteboard_id)


def list_boxes_std(whiteboard_id: str) -> list[Box]:
    with _session() as s:
...
def update_box_std(box_id: str, payload: BoxUpdate) -> Box | None:
    with _session() as s:
        return update_box(s, box_id, payload)


def delete_box_std(box_id: str) -> bool:
    with _session() as s:
        return delete_box(s, box_id)


def list_connectors_std(whiteboard_id: str) -> list[Connector]:
    with _session() as s:
...
def delete_connector_std(connector_id: str) -> bool:
    with _session() as s:
