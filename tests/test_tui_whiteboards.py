...
async def test_drag_on_canvas_pans_scroll_container(db_engine) -> None:
    """Dragging on the canvas should pan the parent ScrollableContainer
    in both X and Y directions via the internal mouse event dispatch."""
    wb = wbs.create_whiteboard_std(WhiteboardCreate(name="wide-tall"))
    wbs.create_box_std(
        BoxCreate(whiteboard_id=wb.id, x=150, y=5, width=10, height=3, label="far")
    )
    ...
    async with app.run_test(size=(40, 20)) as pilot:
        ...
        canvas = screen.query_one("#canvas")
        assert isinstance(canvas, _Canvas)

        start_x, start_y = sc.scroll_x, sc.scroll_y
        assert start_x == 0
        assert start_y == 0

        # Right drag — mouse down at x=35, move to x=25.
        ...
        await pilot.pause()
        assert app.mouse_captured is canvas
        ...
        await pilot.pause()
        assert sc.scroll_x == 10  # dragged 10 left -> reveals content to the right
        assert sc.scroll_y == 0   # no vertical change

        # Diagonal drag — mouse down at (10, 10), move to (5, 5).
        start_x2, start_y2 = sc.scroll_x, sc.scroll_y
        ...
        await pilot.pause()
        assert sc.scroll_x == start_x2 + 5   # dragged 5 left -> +5 reveal
        assert sc.scroll_y == start_y2 + 5   # dragged 5 up   -> +5 reveal
